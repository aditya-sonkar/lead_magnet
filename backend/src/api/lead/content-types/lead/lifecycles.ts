export default {
  async afterCreate(event: any) {
    ...
    // Run email notification in the background so HTTP response returns instantly to user
    setImmediate(async () => {
      try {
        if (strapi.plugin('email') && typeof strapi.plugin('email').service('email').send === 'function') {
          const recipientEmail = process.env.NOTIFICATION_EMAIL || process.env.STRAPI_ADMIN_EMAIL;
          if (recipientEmail) {
